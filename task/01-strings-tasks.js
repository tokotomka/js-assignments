...
function encodeToRot13(str) {
    ...
    let alphabet13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let i;
    let str13 = '';
    for(i=0; i<str.length; i++) {
        let sign = str.charAt(i);
        if(alphabet.includes(sign)) {
            let sign13 = alphabet13.charAt(alphabet.indexOf(sign));
            str13 += sign13;
        } else {
            str13 += sign;
        }       
...
function getCardId(value) {
    const cardSuit = 13;
    return 'A234567891JQK'.indexOf(value.charAt(0)) + cardSuit * '♣♦♥♠'.indexOf(value.slice(-1));
}
